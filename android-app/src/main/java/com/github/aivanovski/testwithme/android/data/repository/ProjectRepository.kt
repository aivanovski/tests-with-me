...
class ProjectRepository(
    ...
    ): Either<AppException, PostProjectResponse> = either {
        api.postProject(request).bind()
    }

    suspend fun getProjects(): Either<AppException, List<ProjectEntry>> = either {
    ...
    suspend fun getProjectByUid(
        uid: String
    ): Either<AppException, ProjectEntry> = either {
        getProjects()
            .bind()
